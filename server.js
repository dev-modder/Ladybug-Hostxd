...
function startBot(sessionId) {
  ...
  // Bot directory - check multiple possible locations (Render.com and local)
  const possibleBotDirs = [
    path.join(__dirname, 'Ladybug-Mini'),              // Cloned inside the project (buildCommand)
    path.join(__dirname, '..', 'Ladybug-Mini'),        // Sibling folder (local)
    '/opt/render/project/src/Ladybug-Mini',            // Render.com with rootDir
    '/opt/render/project/Ladybug-Mini',                // Render.com alternative
    path.join(__dirname, '..', '..', 'Ladybug-Mini'),  // Render.com alternative 2
  ];
  ...
  if (!botDir || !botEntry) {
    log('error', `Bot entry not found. Searched paths:`);
    possibleBotDirs.forEach(dir => {
      const exists = fs.existsSync(dir);
      log('error', `  ${dir} - ${exists ? 'EXISTS' : 'NOT FOUND'}`);
    });
    log('error', 'Running in dashboard-only mode. Check if build command cloned the bot repo.');
...
server.listen(PORT, () => {
  ...
  // Log bot directory search paths
  const startupBotDirs = [
    path.join(__dirname, 'Ladybug-Mini'),
    path.join(__dirname, '..', 'Ladybug-Mini'),
    '/opt/render/project/src/Ladybug-Mini',
    '/opt/render/project/Ladybug-Mini',
  ...
  log('info', 'Bot search paths:');
  startupBotDirs.forEach(dir => {
    const exists = fs.existsSync(dir);
    const hasIndex = fs.existsSync(path.join(dir, 'index.js'));
    log('info', `  ${dir} - exists: ${exists}, has index.js: ${hasIndex}`);
  });
