...
// ─── Bot Download/Installation ──────────────────────────────────────────────────
async function ensureBotInstalled() {
  const possibleBotDirs = [
    path.join(__dirname, BOT_DIR_NAME),
    path.join(__dirname, '..', BOT_DIR_NAME),
    '/opt/render/project/src/' + BOT_DIR_NAME,
    '/opt/render/project/' + BOT_DIR_NAME,
  ];

  // Check if bot already exists
  for (const dir of possibleBotDirs) {
    if (fs.existsSync(path.join(dir, 'index.js'))) {
      log('ok', `Bot found at: ${dir}`);
      return dir;
    }
  }

  // Bot not found - download it
  log('info', 'Bot not found. Downloading from GitHub...');
  
  const installDir = path.join(__dirname, BOT_DIR_NAME);
  ...
  try {
    ...
    try {
      ...
      log('error', 'Git is not installed. Cannot clone bot repository.');
      return null;
    }

    // Clone the repository
    log('info', `Cloning ${BOT_REPO_URL}...`);
    execSync(`git clone --depth 1 ${BOT_REPO_URL} "${installDir}"`, {
      stdio: 'pipe',
      timeout: 120000 // 2 minutes timeout
    });
    
    log('ok', `Bot cloned to: ${installDir}`);
    ...
    log('ok', 'Bot dependencies installed successfully');
    return installDir;
    
  } catch (err) {
    log('error', `Failed to install bot: ${err.message}`);
...
// Global bot directory path
let botDirectory = null;
...
function startBot(sessionId) {
  ...
  // Use the global bot directory (downloaded on startup)
  if (!botDirectory) {
    log('error', 'Bot is not installed. Restart the server to download it.');
    botStatuses.set(sessionId, 'no-bot');
    broadcast('status', { sessionId, status: 'no-bot' });
    return;
  }

  const botEntry = path.join(botDirectory, 'index.js');
  
  if (!fs.existsSync(botEntry)) {
    log('error', `Bot entry not found at: ${botEntry}`);
    botStatuses.set(sessionId, 'no-bot');
    broadcast('status', { sessionId, status: 'no-bot' });
    return;
  }
  
  const botDir = botDirectory;
...
const TEMP_DIRS = [
  ...
  path.join(__dirname, '..', 'Ladybug-Mini', 'tmp'),
  path.join(__dirname, '..', 'Ladybug-Mini', 'downloads'),
  '/opt/render/project/src/Ladybug-Mini/temp',
  '/opt/render/project/src/Ladybug-Mini/tmp',
  '/opt/render/project/src/Ladybug-Mini/downloads',
];
...
// Debug endpoint - shows bot directory status
app.get('/api/debug', (req, res) => {
  const possibleBotDirs = [
    path.join(__dirname, 'Ladybug-Mini'),
    path.join(__dirname, '..', 'Ladybug-Mini'),
    '/opt/render/project/src/Ladybug-Mini',
    '/opt/render/project/Ladybug-Mini',
    path.join(__dirname, '..', '..', 'Ladybug-Mini'),
  ];
  
  const dirStatus = possibleBotDirs.map(dir => ({
    path: dir,
    exists: fs.existsSync(dir),
    hasIndexJs: fs.existsSync(path.join(dir, 'index.js')),
...
// API - Install/Reinstall Bot
app.post('/api/install-bot', async (req, res) => {
  const pin = req.headers['x-dashboard-pin'] || '';
  if (DASHBOARD_PIN && pin !== DASHBOARD_PIN) {
    return res.status(401).json({ error: 'Invalid PIN' });
  }

  log('info', 'Manual bot installation triggered...');
  
  const dir = await ensureBotInstalled();
  if (dir) {
    botDirectory = dir;
    res.json({ ok: true, path: dir, message: 'Bot installed successfully' });
  } else {
    res.status(500).json({ ok: false, error: 'Failed to install bot' });
  }
});
...
server.listen(PORT, () => {
  ...
  // Ensure bot is installed (download if needed)
  ensureBotInstalled().then(dir => {
    if (dir) {
      botDirectory = dir;
      log('ok', `Bot ready at: ${botDirectory}`);
      
      // Run an initial ping
      setTimeout(selfPing, 3000);

      // Auto-start all bots after bot is ready
      setTimeout(startAllBots, 2000);
    } else {
      log('error', 'Bot installation failed. Running in dashboard-only mode.');
      log('info', 'You can manually trigger bot installation via /api/install-bot');
    }
  });
});
